...
/* ── Pagination ── */
.pagination{padding:20px 0;display:flex;flex-direction:column;align-items:center;gap:12px}
.page-info-text{font-size:13px;color:var(--muted)}
.pag-btns{display:flex;align-items:center;gap:12px}
.pag-btn{padding:8px 18px;border-radius:8px;border:1px solid var(--border);background:var(--surface);
...
.pag-btn.primary:hover:not(:disabled){opacity:.85}
.pag-dots{display:flex;gap:6px;align-items:center}
.dot{width:8px;height:8px;border-radius:50%;background:var(--border);cursor:pointer;transition:all .15s}
.dot.active{background:var(--blue);transform:scale(1.25)}
.dot:hover{background:var(--blue);opacity:.6}
...
<div class="wrap">
  ...
  <div class="pages-section" id="pages-container">${pagesHtml}</div>

  ${totalPages > 4 ? `<div class="pagination" id="pagination">
    <div class="page-info-text">Showing pages <strong id="pag-from">1</strong>-<strong id="pag-to">${Math.min(4, totalPages)}</strong> of <strong>${totalPages}</strong></div>
    <div class="pag-btns">
      <button class="pag-btn" id="pag-prev" disabled>&larr; Previous</button>
      <div class="pag-dots" id="pag-dots"></div>
      <button class="pag-btn primary" id="pag-next">Next &rarr;</button>
    </div>
  </div>
  <script>
  (function(){
    var PER=4,cur=0;
    var blocks=document.querySelectorAll('#pages-container .page-block');
    var total=blocks.length;
    var pages=Math.ceil(total/PER);
    var prev=document.getElementById('pag-prev');
    var next=document.getElementById('pag-next');
    var dots=document.getElementById('pag-dots');
    var from=document.getElementById('pag-from');
    var to=document.getElementById('pag-to');
    function render(){
      var s=cur*PER,e=Math.min(s+PER,total);
      for(var i=0;i<total;i++) blocks[i].style.display=(i>=s&&i<e)?'':'none';
      from.textContent=s+1;to.textContent=e;
      prev.disabled=cur===0;next.disabled=cur>=pages-1;
      var ds=dots.querySelectorAll('.dot');
      for(var j=0;j<ds.length;j++) ds[j].className=j===cur?'dot active':'dot';
    }
    for(var p=0;p<pages;p++){
      var d=document.createElement('span');d.className=p===0?'dot active':'dot';
      d.setAttribute('data-p',p);
      d.onclick=function(){cur=parseInt(this.getAttribute('data-p'));render();};
      dots.appendChild(d);
    }
